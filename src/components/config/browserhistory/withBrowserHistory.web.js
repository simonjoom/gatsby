...
export default function withBroserHistory(Navigator) {
  const Wrapper = class extends Component {
    ...
    componentDidMount() {
      ...
      const initialPath = this.cleanPathWithBaseUrl(
        window.location.href.replace(uriPrefix, '')
      )
      ...
      this.history.listen((location, action) => {
        if (action === 'POP') {
          ...
          let path = this.cleanPathWithBaseUrl(pathname + search)
          const navigationAction = Navigator.router.getActionForPathAndParams(
            path
          )
          this.dispatch({
            ...navigationAction,
    ...
    setNavFromPath = path => {
      ...
      const action =
        Navigator.router.getActionForPathAndParams(pathWithoutQuery, params) ||
    ...
    dispatch = action => {
      if (typeof action === 'function') {
        if (this.props.getState)
          return action(this.dispatch, this.props.getState)
