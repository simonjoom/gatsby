...
class Helper extends Component {
  ...
  prepareFordtb = (object, iscreate, getkey = false, disconnectid = false) => {
    ...
    Object.keys(object).map((key, index) => {
      ...
      if (isInt(placehold)) {
        outobj[key] = parseInt(outobj[key], 10)
      }
    })
  ...
  validateFields(testmore) {
    ...
    if (this.state.fields) {
      ...
      const bool = array.every(key => {
        const filterNumber =
          !isInt(this.props.placeholder[key]) ||
          parseInt(this.state.fields[key], 10) ||
          this.state.fields[key] == '0'
        if (!filterNumber && this.state.fields[key])
          alert('Enter Number', this.props.placeholder[key])
        return (
          (!!this.state.fields[key] ||
            !isRequired(this.props.placeholder[key])) &&
          filterNumber
        )
      })
  ...
  fetchState(fields, selected, tofetch) {
    ...
    if (this.props.initfetch) {
      Object.keys(fields).map((key, index) => {
        var n = fields[key]
        datas[key] = n == null ? this.props.initfetch[key] : n
      })
    } else {
      datas = fields
    }
  ...
  render() {
    ...
    return (
      <KeyboardAwareCenteredView style={{ width: 500 }}>
        {this.state.fields &&
          this.renderFields(
            this.state.fields,
            placeholder,
